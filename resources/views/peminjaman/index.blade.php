...
@section('content')
    ...
    <div x-data="{ openReturn: null, deleteConfirm: false, deleteId: null, deleteCode: '' }" class="w-full min-h-screen flex flex-col">
        ...
        {{-- ================= FILTER (UPDATED SESUAI FOTO & CONTOH) ================= --}}
        <form method="GET" action="{{ route('peminjaman.index') }}" class="mb-6">
            <div class="p-4 rounded-2xl shadow-lg flex flex-wrap items-center gap-3 backdrop-blur-md"
                ...
                {{-- INPUT SEARCH --}}
                <input type="text" name="search" value="{{ request('search') }}" placeholder="🔍 Cari Tools atau Karyawan..."
                ...
                {{-- TOMBOL FILTER (STYLE BARU ROUNDED FULL) --}}
                <button type="submit"
                ...
                {{-- TOMBOL RESET (STYLE BARU ROUNDED FULL) --}}
