...
@section('content')
    <div class="max-w-7xl mx-auto p-6" x-data="{ openTools: false }" @close-modal.window="openTools = false">
        ...
        <form action="{{ route('peminjaman.store') }}" method="POST">
            ...
            <div class="bg-white shadow rounded-xl p-6 mb-6">
                ...
                <div class="grid md:grid-cols-3 gap-6">
                    <input type="text" name="client_name" class="border rounded-lg px-3 py-2" placeholder="Nama Client">
            ...
            <div class="bg-white shadow rounded-xl p-6">
                ...
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button type="button" @click="openTools = true"
                        class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
                        + Pilih Tools
                    </button>
                </div>

                <table class="w-full text-sm border rounded-lg overflow-hidden" id="tableSelectedTools">
        ...
        {{-- ================= MODAL ================= --}}
        <div x-show="openTools" x-transition @click.self="openTools = false" @keydown.escape.window="openTools = false"
            class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">

            <div class="bg-white w-3/4 rounded-xl shadow-xl p-6 max-h-[80vh] overflow-y-auto">

                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold">Pilih Tools</h3>
                    <button @click="openTools = false" class="text-2xl text-gray-500">&times;</button>
                </div>

                <input type="text" id="searchTools" class="w-full border rounded-lg px-3 py-2 mb-4"
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody id="toolsTable">
                        @foreach ($serials as $serial)
                                        <tr class="border-b">
                                            <td>
                                                <input type="checkbox" class="tool-checkbox" data-id="{{ $serial->id }}"
                                                    data-name="{{ $serial->toolkit->toolkit_name }}"
                                                    data-serial="{{ $serial->serial_number }}" data-image="{{ $serial->toolkit->image }}">
                                            </td>

                                            <td>
                                                <img src="{{ $serial->toolkit->image
                            ? asset('storage/' . $serial->toolkit->image)
                            : asset('images/no-image.png') }}" class="preview-image w-12 cursor-pointer">
                                            </td>
                ...
                <div class="flex justify-end gap-3 mt-4">
                    <button @click="openTools = false" class="px-4 py-2 border rounded-lg">
                        Batal
                    </button>

                    <button type="button" id="btnAddTool" class="bg-blue-600 text-white px-4 py-2 rounded-lg">
                        + Tambahkan
                    </button>
                </div>
            </div>
        ...
        {{-- ================= IMAGE PREVIEW ================= --}}
        <div id="imagePreviewModal" class="fixed inset-0 bg-black/70 hidden z-50 flex items-center justify-center">
            <div class="relative">
                <button id="closePreview" class="absolute -top-8 right-0 text-white text-2xl">✕</button>
                <img id="previewImage" src="" class="max-h-[90vh] max-w-[90vw] rounded shadow-lg">
            </div>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('btnAddTool').addEventListener('click', function () {
            ...
            <tr id="row-${id}" class="border-b">
                ...
                <td class="p-2">
                    <img src="${imagePath}" class="w-12 cursor-pointer preview-image">
                </td>
